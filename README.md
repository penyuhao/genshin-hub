...
| 画廊管理 | **每屏独立定制**：标题/副标题/描述、背景图（可直接上传）、**背景视频（上传或填直链，含静音 / 循环 / 不透明度开关）**、字体、标题颜色、**9 种标题特效**（光幕/渐变/霓虹/描边/错位/纯色/逐字波浪/故障风/极光）、**水平对齐**（左中右）、**垂直位置**（上中下）、**位置微调 ±45%**、**字号倍率 0.5~1.8**、**本屏遮罩强度**、**Ken Burns 开关**、按钮文案与跳转；支持增删与排序 |
| **页面背景** | **给每个界面单独配背景**：首页内容区 / 下载页 / 功能页 / 关于页；背景图（可上传）+ 覆盖色 + **遮罩装饰图**（8 张内置素材可直接用），支持混合模式、平铺方式、模糊、压暗与固定视差 |
...
npm run test:full         # 冒烟 + 上传链路 + 后台保存 + 前端 DOM + 子页面刷新 + SSE 全跑
npm run test:api          # 后端接口冒烟（PowerShell，Windows）
npm run test:admin        # 后台「点保存 → 真的存下去」端到端（自带隔离实例）
...
**Q：后台改了设置、按了保存，结果没有反应 / 不生效？**
A：这是 v2.6.3 修掉的一个大问题：**「保存配置」按钮以前是个摆设**。
它被放在区块标题栏里，也就是 `<form>` 的兄弟节点，而 `type="submit"` 的按钮只有在能解析出
form owner 时才会提交表单 —— 否则点击等于什么都没发生，界面还一点报错都不给。
受影响的正是所有通用区块（站点设置 / 画廊管理 / 主题编辑 / 导航管理 / 功能开关 / 背景音乐 /
下载管理 / 关于编辑 / 快捷入口 / 字体管理）。现在已经显式绑定提交，并新增
`npm run test:admin`（真实点击按钮 → 校验服务端配置确实变了）防止回归。
如果你在旧版本里改过这些设置，需要**重新改一次并保存** —— 那些改动当时并没有写进配置文件。

**Q：后台填了链接，为什么前台还显示"链接待补充"？**
A：v2.6.2 已修。旧版链接校验只认 `https://…` 或站内 `/path`，**只写域名（例如 `ys.mihoyo.com`）会被判不合格**；
而下载卡片是整段提交的，一个字段不合格就整段保存失败，旧后台又只有一条 5 秒的 toast，很容易以为存上了。
现在：只写域名会自动补 `https://`、`http://` 会自动升级成 `https://`；保存失败时后台会在表单顶部
列出「哪个字段、什么原因」并把输入框标红滚到眼前。链接类输入框失焦时也会当场补全，所见即所存。

**Q：从页面底部往上滚，底部会空出一条背景？**
A：v2.6.2 已修。原因是页面还停在下方内容区时，只要鼠标落在画廊区域，滚轮就被画廊接管去翻屏 ——
画廊只露出一半、内部却自顾自翻页，底下那条空白就一直挂着。现在**页面没回到画廊顶部时画廊不抢滚轮**，
先把页面滚回去，回到顶部才恢复"一次手势一屏"。另外「↑ 返回画廊」现在会同时把页面带回画廊顶部。
...
- **顶栏开源仓库图标**（可在后台配置地址并一键开关）
  ...
  - 地址在「站点设置 → 开源仓库地址」里填，**默认就是本项目的仓库** `https://github.com/penyuhao/genshin-hub`
  - 「功能开关 → 顶栏开源仓库图标」可随时关掉；地址留空或关掉开关时图标不显示
  - 悬停文案可在「开源图标提示文案」里改（默认「GitHub 开源仓库」）
- 测试：DOM 新增 5 项（指向仓库、新窗口+noopener、内联 SVG、后台字段与 schema、关闭时隐藏），共 **210 项**

### v2.13.0
- **背景图可以放多张**：一个界面可配多张图，按设定秒数**交叉淡入淡出轮播**（4~300 秒）
  - 后台「多张背景图」一行一个地址；填了它就以它为准，单张的 `image` 作为兜底
  - 切到后台标签页会暂停轮播，减少无谓消耗
- **「从媒体库选」：不用再自己拼地址**（专治 Docker / NAS 部署时"地址很难获取"）
  - 后台每个图片字段旁边多了「媒体库」，列出 `DATA_DIR/uploads/images`、`uploads/videos` 里已有的文件，**点一下就填好**
  - 也可以直接用文件管理器 / NAS 界面把图丢进挂载卷的 `data/uploads/images/`，刷新媒体库就能选到
  - 接口 `GET /api/uploads/list`（管理员），返回可直接使用的 `/uploads/...` 地址 + 目录位置提示
- 测试：uploads 新增 6 项（多张背景图持久化、轮播间隔下限、媒体库鉴权/内容/目录提示），共 **53 项**

### v2.12.0
- **每个国家/地区一个图标**（照官方地图那种「图标 + 名字」的排法）
  - 内置 **11 个图标**（`frontend/images/icons/`）：原神、蒙德(风)、璃月(岩)、稻妻(雷)、须弥(草)、枫丹(水)、纳塔(火)、至冬(冰)、挪德卡莱、坎瑞亚、哥伦比娅
  - 首页「提瓦特索引」卡片改成：**图标在左、名字与副标题在右**，更接近官方地图的观感
  - 后台「画廊管理」每一屏可单独指定图标；留空则**按标题自动匹配**内置图标，也可以上传自己的
- 测试：DOM 新增 6 项图标断言（11 个素材存在、可访问、后台字段与 schema），共 **205 项**

### v2.11.0
- **监控面板按 Kuma 的分组显示**（保留分组，顺序在 Kuma 里排）
  后端本来就按 Kuma 状态页 `publicGroupList` 的顺序拍平监控、并把分组名带在每条数据上，但前台把分组丢掉了。
  现在照原样**分块显示**：分组标题 + 每组数量，卡片顺序就是你在 Kuma 里排的顺序。
  想调顺序或换分组，直接在 Uptime Kuma 的状态页里调即可，站点这边不用再配一遍。
- **画廊按钮可以填网页地址**：每屏新增「按钮跳转到的网页地址」——外链新窗口打开（带 `rel="noopener noreferrer"`）、
  站内路径当前窗口打开；留空仍按「跳转到站内视图」走
- **背景音乐**：修了"填了地址没声音"——真正原因是「功能开关 → 背景音乐」没打开（配置与音频文件都没问题）。
  现在浏览器拦下自动播放时不再静默（显示 ♪ 按钮，用户第一次点击/按键会自动重试），
  站点体检也会明确报出「配了音乐地址但开关是关的」与「地址看起来不是音频文件」
- **后台"太杂"→ 折叠卡片**：画廊 11 屏、背景 5 个界面这类长列表改成折叠卡片，
  标题带可读名称 + 一句话摘要（"特效 · 对齐 · 有视频背景"），上方显示"共 N 项"，上移/下移/删除不会误触折叠
- 测试：DOM 套件新增 3 项分组断言（直接对着真实 Kuma 数据校验分组名与顺序），共 **199 项**

### v2.10.1
- **再修：从内容区往上滚，画面会卡在"画廊只露出一小半"的过渡带**
  上一版只处理了**滚轮**这条路径（一步落进画廊内部就直接对齐到画廊顶部）+ 页面级 CSS 吸附。
  但**触摸滑动、拖滚动条、键盘 PageUp、触控板惯性**都不走滚轮事件，CSS 吸附也不是所有浏览器/场景都生效，
  所以仍可能停在过渡带：上半是坎瑞亚（最后一屏）被切掉一半的空背景，下半是内容区开头的空白。
  现在加了一道**兜底**：滚动**停下来**之后（160ms 没有滚动事件）只要位置落在过渡带里，
  就平滑对齐到最近的一头 —— 画廊顶部或内容区顶部。这条路径与手势类型无关，全部覆盖。
- 顺带：对齐回画廊时**重新同步一次"当前屏"**，避免出现"画面是坎瑞亚、右侧圆点却没高亮、月亮也不在"的状态
  （「↑ 返回画廊」按钮同样处理）
- 测试：DOM 套件新增 2 项针对这条兜底的用例（靠上归画廊 / 靠下归内容区），共 **192 项**

### v2.10.0
- **修复：背景图地址里带 `..` 会被前端静默丢掉（"设了没反应"）**
  `safeCssUrl()` 里那句"含 `..` 就拒绝"本来是防站内路径穿越的，结果把**外链**也一起误伤了 ——
  官方 CDN 的图就长这样：`…/images/home@1x.78a07083..jpg?x-oss-process=…`，
  于是配置存得进去、页面上什么都不显示。现在 `..` 检查只对**站内相对路径**生效，外链原样放行。
- **新增「站点体检」**（后台 → 系统 → 站点体检；接口 `GET /api/diagnostics`，仅管理员）
  一键把"哪里有问题"列成清单，分三级（✖ 需要处理 / ⚠ 建议看看 / ✔ 正常），每条都带**具体怎么改**：
  - **运行环境**：数据目录可写、上传目录、配置备份数量、数据源是否还是 Mock、验证码开关、当前协议、SSE 连接数、版本
  - **配置引用**：画廊背景图/背景视频、页面背景图与遮罩、Logo/favicon、背景音乐、快捷入口、下载卡片 ——
    站内路径**直接查文件在不在**，外链发 HEAD 看能不能访问（去重 + 最多 40 个 + 并发 4 + 6 秒超时，不拖慢站点）
  - **常见坑**：背景填成了网页地址而不是图片、同一界面配了多份背景、某屏既没图也没视频、导航视图重复、必填项为空
  - **混合内容**：整站 https 时配置里还留着 `http://` 资源 —— 浏览器会直接拦掉，这里报"需要处理"

### v2.9.0
- **管理后台重做易用性**（针对"不够直观、改起来太难"）
  - **字段分组折叠**：区块内按「基本 / 标题样式 / 排版位置 / 背景与可读性 / 按钮」等分组折叠，
    打开先看到最常用的几项 —— 画廊单屏原来 18 个输入框平铺，现在只摊开"基本"
  - **0~1 这类数值改成滑杆**（遮罩强度、不透明度、微调、字号倍率…），带实时数值，拖着看效果
  - **遮罩可视化选择器**：8 张内置遮罩点缩略图即可选用，不用手打路径，选中的高亮；也可一键"不用遮罩"
  - **侧栏按用途分成三组**（内容 / 外观 / 系统），每个区块下面加一句话说明（"画廊每一屏的字与图"）
  - **数组条目显示可读名称**：背景项显示"全局（星空那一层）/ 下载页"，画廊项直接显示屏标题，不再只是"第 N 项"
  - **区块标题栏吸顶**：长表单滚到哪儿都能点保存
- **可以从官方站点"取图"**：图片字段新增「**从链接导入**」
  在官方站点右键图片 →「复制图片地址」→ 粘进导入框，服务端会把图片**下载并保存到本站**
  （比直接外链更稳，不受对方 CDN 的 referer 策略影响）。
  安全上：只允许 http/https 且**拒绝解析到内网/回环的地址**（SSRF 防护，必要时可 `ALLOW_PRIVATE_IMAGE_IMPORT=true` 放开）、
  手动跟随最多 3 次跳转且每跳重新校验、落盘前用**魔数**确认它真的是图片（不看 Content-Type 脸色）
- **新增「全局（星空那一层）」背景**：官方美术图当底、**星星粒子叠在图上**（靠 DOM 顺序分层）。
  默认给了一层柔和暗角，让页面边缘收得住

### v2.8.0
- **页面背景高度自定义**（不只是画廊，几乎每个界面都能单独配）
  后台新增「**页面背景**」区块，可选界面：**首页画廊下方内容区 / 下载页 / 功能页 / 关于页**，每个界面一层外观：
  | 可调项 | 说明 |
  |---|---|
  | **背景图** | 可直接上传或填外链；留空则保持透明（继续看得到星空） |
  | **覆盖色 + 不透明度** | 在背景图之上盖一层色，用来统一色调 |
  | **遮罩 / 装饰图** | 叠在最上面的 PNG / SVG（自带透明通道最好用）：点阵、网格、光线、暗角… |
  | **遮罩混合模式** | `screen` 发光 / `overlay` 提对比 / `soft-light` 柔和 / `multiply` 压暗 / `luminosity` 取明暗 / `normal` |
  | **遮罩铺法** | 平铺成纹理（小图推荐）/ 拉伸铺满 / 完整显示一张 |
  | **背景模糊 / 压暗** | 0~24px 模糊、0~1 压暗 |
  | **背景固定** | 滚动时不动，做出视差感 |
  - 内置 **8 张遮罩素材**（随仓库发布，直接填路径即可）：`/images/masks/` 下的
    `dots.svg`（点阵）、`grid.svg`（方格）、`lines.svg`（斜纹）、`rays.svg`（放射光）、
    `waves.svg`（波浪）、`vignette.svg`（暗角）、`hex.svg`（蜂窝）、`sparkle.svg`（星芒）
  - 默认已给四个界面配上低调的纹理遮罩（不遮挡星空背景），想更花/更干净都在后台改
- **修复：从最底下往上滚时的"拼贴"画面**
  最后一步如果会停在"画廊只露出一小半"的位置，画面就会是**坎瑞亚（最后一屏）被截掉下半截的空背景 + 内容区开头的空白**。
  现在两处一起解决：① 滚轮回退时若这一步会落进画廊内部，**直接对齐到画廊顶部**；
  ② 首页启用**页面级滚动吸附**（锚点＝画廊顶部 / 内容区顶部，`proximity` 只在停下时离得近才吸附），
  这样触摸滑动、键盘、拖滚动条也一样不会停在半路。
  逐帧滚动动画期间会临时关闭吸附，避免被浏览器每帧拽回锚点。
...
- **修复（重要）：后台「保存配置」按钮点下去根本没反应 —— 所有通用区块都存不进去**
  ...
  时才会提交表单；否则点击等于什么都没发生 —— 而且**界面上一点报错都没有**，
  于是"改了设置 → 按保存 → 没反应 / 不生效"。
  受影响的是所有用通用表单渲染的区块：站点设置、画廊管理、主题编辑、导航管理、功能开关、
  背景音乐、下载管理、关于编辑、快捷入口、字体管理（数据源设置与账号安全有自己的按钮，所以一直是好的）。
  现在保存按钮显式绑定到同一个提交函数（表单内回车提交照旧）。
- **新增 `tests/admin-save.mjs`：真的去点那个按钮的端到端测试**（14 项）
  自带隔离实例 + 真实 jsdom，验证「点保存 → 服务端配置真的变了 + 生成了备份」、
  「连续保存第二次也生效」、「非法链接会显示错误框并标红且不写入」。这个测试能直接抓住上面那类 bug。
- **后台区块切换加防竞态**：切换是异步的，旧请求回来时不再覆盖已经切过去的新区块
  （表现是"点了某个区块，闪一下又变回上一个"）。
- **配置读取去掉 5 分钟 localStorage TTL**：以前缓存没过期就直接用缓存、根本不问服务端，
  于是后台改完配置、另一个标签页或刚刷新的页面在 5 分钟内还是旧内容（另一种"配置不生效"）。
  现在始终以服务端为准（接口本身带 ETag，没变就是 304），本地缓存只在接口不可用时兜底；
  另外后台保存后会广播一个版本号，**其它已打开的标签页也会自动同步**。
- **保存失败看得见**：失败时表单顶部常驻错误框（列出「字段路径：原因」）+ 出问题的输入框标红并滚到眼前；
  链接类输入框失焦自动补 `https://`；后端对裸域名做归一化（只写 `ys.mihoyo.com` 也能存）。
- **写入更稳**：配置文件 / 凭据 / 数据源设置的原子写入加了 rename 重试 ——
  Windows 杀毒、索引服务或容器卷偶尔会短暂占用刚写完的临时文件（EPERM/EBUSY），
  以前会随机表现为"保存失败"。
- **版本可见**：`/health` 返回版本号，后台侧栏底部直接显示当前后台版本，排查"我是不是没更新上"一眼就能确认。

### v2.6.2
- **修复："我明明补充了链接，却显示链接待补充"**
  根因：链接校验只认 `https://…` 或站内 `/path`，**只写域名（`ys.mihoyo.com`）会被拒绝**；
  而下载卡片是整段提交的，一个字段不合格 → 整个区块保存失败；旧后台又只用一条 5 秒就消失的 toast 提示，
  很容易以为保存成功了，页面于是继续显示旧值。
  现在三处一起改：
  1. **后端宽容归一化**：只写域名自动补 `https://`、`http://` 自动升级为 `https://`、`//host` 自动补协议；
     `javascript:` / `data:` / 路径穿越 / 含空格的地址仍然拒绝
  2. **后台常驻错误框**：保存失败时在表单顶部列出「字段路径：原因」，并把出问题的输入框**标红 + 滚动到视野内**，
     不再只有一闪而过的 toast（`frontend/js/admin.js#showFormError`）
  3. **输入框失焦即补全**：链接类字段（`url` / `bgImage` / `bgVideo` / `logo` / `favicon`）失焦时自动补 `https://`，
     所见即所存
  顺带修掉 `collectForm()` 会把数组条目里的值（如最后一张卡片的 `url`）错误地写到顶层的串值问题。
- **修复：从页面底部往上滚时"空出来一条背景"**
  根因：页面还停在下方内容区（`window.scrollY > 0`）时，只要鼠标落在画廊区域，滚轮就被画廊接管去翻屏 ——
  画廊只露出一半、内部却自顾自翻页，视口底部那条下方内容区的空白就一直在。
  现在**页面没回到画廊顶部时，画廊不抢滚轮**，完整交还给页面；回到顶部后才恢复"一次手势一屏"。
  同时把窗口滚动改成自己控制的逐帧缓动（并移除 CSS 的 `scroll-behavior: smooth`，避免浏览器平滑滚动
  与逐帧写入互相打断、滚一下只挪一点点）；「↑ 返回画廊」现在会**同时**把页面带回画廊顶部（以前只重置了画廊内部）；
  横屏手机也从 `100svh` 改为 `100dvh`（地址栏隐藏时底部不再漏背景）。
...
- **画廊从"统一模板"升级为"逐屏自由定制"**（解决"太单一 + 不够自定义"）
  ...
  - **自由定位**：`offsetX` / `offsetY` 百分比微调（±45%），文字想放哪就放哪
  - **字号倍率** 0.5~1.8、**逐屏遮罩强度**、**逐屏 Ken Burns 开关**
  - 11 屏出厂即配好不同组合：特效 6 种、对齐 3 种、垂直 3 种，**没有两屏观感相同**
  - 全部在后台「画廊管理 → 每一屏」里点选，保存即刻生效
- **SSE 测试更贴合真实数据源**：真实 Kuma 检查间隔可达 60s，后端"仅在数据变化时推送"属设计行为；
  测试现在区分「窗口内无变化（保活帧正常）」与「连接异常」，不再误报失败
