...
- 🎴 **11 屏沉浸式画廊**：开场「原神」+ 七国（蒙德/璃月/稻妻/须弥/枫丹/纳塔/至冬）+ 挪德卡莱 + 坎瑞亚 + 哥伦比娅；**纵向堆叠、原生滚动**（一条滚到底，不劫持滚轮、刷新后位置正确），桌面端右侧圆点可平滑跳转
...
| 11 屏纵向滚动画廊 | **11 屏**：开场「原神」+ 七国（蒙德/璃月/稻妻/须弥/枫丹/纳塔/至冬）+ 挪德卡莱 + 坎瑞亚 + 哥伦比娅角色特写；**每屏一屏高、自然堆叠，原生滚动** | `frontend/js/gallery.js` |
...
- **首页画廊下方内容**：仅两个模块 —— 「服务状态速览」（实时在线/异常数 + 进入完整面板）与「快捷入口」（外链卡片）。
...
9. **首页画廊改为主界面「一条滚到底」**：早期实现是"锁在一屏内切换"（劫持滚轮 + 内部索引），
   在手机上和刷新后容易出现定位/体验问题；现改为 **11 屏纵向堆叠的原生滚动** ——
   不劫持滚轮、每屏 `min-height: 100svh`、滚动位置由浏览器正确恢复。
   圆点导航与下滚提示移到 body 层级（视图的 `transform` 动画会让 `position: fixed` 以视图为基准，这是个经典坑）。
   逐字浮现、Ken Burns、月亮呼吸等效果保留（改为进入视口时触发，只对当前屏播放以省电）。
...
**Q：手机上看首页是什么体验？**
A：一条滚到底。11 屏纵向堆叠，每屏一屏高，用手指自然下滑即可；圆点导航在手机上自动隐藏（平板上也是），
首屏有「↓ 向下浏览」提示，滚过首屏自动消失。月亮只在首屏出现，标题字号按屏幕宽度收敛不会溢出。

**Q：为什么首页不是"一屏一屏切换"了？**
A：早期版本是锁屏切换（劫持滚轮 + 内部索引），在手机上别扭、刷新后位置也容易错乱。
现在改为原生滚动：不劫持任何滚动事件，滚动位置由浏览器管理，刷新/前进后退都正确。
...
### v2.1（当前）
- **首页交互重做：一条滚到底的原生滚动**
  - 11 屏改为纵向堆叠（每屏 `min-height: 100svh`），不再劫持滚轮、不再"锁屏切换"，刷新后滚动位置正确
  - 圆点导航 / 下滚提示移到 body 层级 —— 修复视图 `transform` 动画导致 `position: fixed` 以视图为基准的定位错乱
  - 逐字浮现、Ken Burns、月亮改为"进入视口触发 / 当前屏才播放"，手机端更省电
  - 手机端专门优化：首屏给月亮留位、其余屏内容垂直居中、标题字号收敛不溢出、状态指标 2×2 排列、平板及以下隐藏圆点
- **可移植性：部署到任何地方**
