...
| 4 屏纵向滚动画廊 | **11 屏**：开场「原神」+ 七国（蒙德/璃月/稻妻/须弥/枫丹/纳塔/至冬）+ 挪德卡莱 + 坎瑞亚 + 哥伦比娅角色特写；滚轮 / 触摸 / 圆点 / 方向键 | `frontend/js/gallery.js` |
| 画廊下滑一步跳转 | 末屏继续下滑或点 ↓ → 平滑跳到下方「服务状态速览」，不再逐像素拖动页面 | `gallery.js#onExitDown` + `main.js#scrollToHomeContent` |
...
9. **画廊下滑改为"一步跳转"**：末屏继续下滑（或点 ↓ / 按方向键）直接平滑跳到画廊下方的「服务状态速览」，不再逐像素滚动；下方向上滚动则一步回到画廊。
