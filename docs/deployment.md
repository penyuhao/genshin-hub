...
> **访客不用强刷**：前端 js/css/字体走 `no-cache` + ETag（没变返回 304），所以你更新完、重启进程之后，
> 访客**普通刷新就是新版**，不需要通知大家按 Ctrl+F5。若前面挂了 CDN，请确认 CDN 尊重 `Cache-Control` 与 `ETag`。
> 另外注意：服务端代码改动必须重启进程才生效（开发时用 `npm run dev`，它带 `node --watch`）。
